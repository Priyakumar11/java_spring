...
public class UserService {
	...
	public User getUser(String authorization) {
		...
		System.out.println("authorization.split(\" \")[0]:"+authorization.split(" ")[0]);
		LoginSession loginSession = loginSessionDao.findByAccessToken(authorization.split(" ")[0]);
		if (loginSession == null) {
			throw new CustomException("Invalid access loginSession.", HttpStatus.UNAUTHORIZED);
		}
		User user = userDao.findById(loginSession.getUserId());
		if (user == null || user.getIsAdmin() == 'N') {
			throw new CustomException("Invalid access user id.", HttpStatus.UNAUTHORIZED);
		}
		return user;
	}
